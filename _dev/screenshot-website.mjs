...
async function run() {
  ...
  const totalHeight = await desktopPage.evaluate(() => document.body.scrollHeight);
  ...
  const metadata = await desktopPage.evaluate(() => {
    ...
    const allLinks = Array.from(document.querySelectorAll('nav a, header a')).map(a => ({
    ...
    const sections = Array.from(document.querySelectorAll('section, [class*="section"]')).map((s, i) => ({
      index: i,
