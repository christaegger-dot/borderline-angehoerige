...
async function getLayoutData(page) {
  return await page.evaluate(() => {
    ...
    const editorialLayout = document.querySelector('[class*="editorial"]') ||
    ...
    function domTree(el, depth = 0) {
      ...
      const cls = el.className ? ` class="${el.className.substring(0, 120)}"` : '';
    ...
    const toc = document.querySelector('[class*="toc"], [class*="TableOfContents"], aside');
    if (toc) {
      const tocRect = toc.getBoundingClientRect();
      anomalies.push(`TOC/Sidebar present: ${toc.tagName} class="${toc.className.substring(0,100)}" width=${Math.round(tocRect.width)}px`);
    }

    // 2. Cards with backgrounds/shadows
    const cards = document.querySelectorAll('[class*="card"], [class*="Card"], [class*="bg-sage"], [class*="bg-teal"], [class*="shadow"]');
    const cardInfo = [];
    cards.forEach((card, i) => {
      if (i > 10) return;
      const cs = window.getComputedStyle(card);
      const hasBg = cs.backgroundColor !== 'rgba(0, 0, 0, 0)' && cs.backgroundColor !== 'rgb(255, 255, 255)';
      const hasShadow = cs.boxShadow !== 'none';
      if (hasBg || hasShadow) {
        cardInfo.push(`  - ${card.tagName} class="${card.className.substring(0,80)}" bg=${cs.backgroundColor} shadow=${hasShadow}`);
      }
    });
    if (cardInfo.length > 0) {
      anomalies.push('Card-Wrapper mit Hintergrund/Schatten:\n' + cardInfo.join('\n'));
    }
    ...
    allDivs.forEach((div, i) => {
      ...
      if (rect.height > 200 && div.textContent.trim().length < 10 && div.children.length === 0) {
        emptyBlocks.push(`  - ${div.tagName} class="${div.className.substring(0,80)}" h=${Math.round(rect.height)}px`);
      }
    });
    ...
    const sageElements = document.querySelectorAll('[class*="sage"], [class*="teal-wash"], [class*="bg-teal"]');
    if (sageElements.length > 0) {
      const sageInfo = Array.from(sageElements).slice(0, 5).map(el =>
        `  - ${el.tagName} class="${el.className.substring(0,80)}"`
      ).join('\n');
      anomalies.push(`Sage/Teal-Token-Reste (${sageElements.length} Elemente):\n${sageInfo}`);
    }
