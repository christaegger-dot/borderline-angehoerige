...
async function getLayoutData(page) {
  return await page.evaluate((safeClassStr) => {
    ...
    if (toc) {
      ...
      anomalies.push(`TOC/Sidebar: ${toc.tagName} class="${safeClass(toc)}" w=${Math.round(tocRect.width)}px left=${Math.round(tocRect.left)}px`);
    }
    ...
    allElements.forEach((el) => {
      ...
      if ((hasBg || hasShadow) && hasBorderRadius) {
        ...
        if (rect.width > 100 && rect.height > 50) {
          cardInfo.push(`  - ${el.tagName} class="${safeClass(el).substring(0,80)}" bg=${cs.backgroundColor} shadow=${hasShadow} br=${cs.borderRadius}`);
          cardCount++;
        }
      }
    });
    if (cardInfo.length > 0) {
      anomalies.push('Card-Wrapper mit Hintergrund/Schatten/BorderRadius:\n' + cardInfo.join('\n'));
    }
    ...
    allElements.forEach((div, i) => {
      ...
      if (rect.height > 200 && div.textContent.trim().length < 5 && div.children.length === 0) {
        emptyBlocks.push(`  - ${div.tagName} class="${safeClass(div).substring(0,80)}" h=${Math.round(rect.height)}px`);
      }
    });
    if (emptyBlocks.length > 0) {
      anomalies.push('Leere Whitespace-Blöcke (>200px):\n' + emptyBlocks.join('\n'));
    }
    ...
    if (main) {
      ...
      if (asymmetry > 30) {
        anomalies.push(`Layout-Asymmetrie: left=${leftMargin}px right=${rightMargin}px (diff=${asymmetry}px)`);
      } else {
        anomalies.push(`Layout OK: left=${leftMargin}px right=${rightMargin}px (diff=${asymmetry}px)`);
      }
    }

    // 5. Sage/teal backgrounds
    const sageElements = Array.from(document.querySelectorAll('*')).filter(el => {
      const cls = safeClass(el);
      return cls.includes('sage') || cls.includes('teal-wash') || cls.includes('bg-teal');
    }).slice(0, 5);
    if (sageElements.length > 0) {
      const sageInfo = sageElements.map(el => `  - ${el.tagName} class="${safeClass(el).substring(0,80)}"`).join('\n');
      anomalies.push(`Sage/Teal-Token-Reste (${sageElements.length}):\n${sageInfo}`);
    }
    ...
    if (firstChild) {
      ...
      if (centerOffset > 50) {
        anomalies.push(`Inhalt NICHT zentriert: center-offset=${Math.round(centerOffset)}px (firstChild left=${Math.round(rect.left)}px)`);
      } else {
        anomalies.push(`Inhalt zentriert: center-offset=${Math.round(centerOffset)}px`);
      }
    }
...
async function run() {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const pg of PAGES) {
      ...
      try {
        ...
        results[key] = data;
        console.log(`  ✓ Anomalien: ${data.anomalies.length}, pageHeight: ${data.pageHeight}px`);
      } catch (err) {
  ...
  await browser.close();
  writeFileSync(join(OUT_DIR, 'raw-results.json'), JSON.stringify(results, null, 2));
