...
const OUT_DIR = '/home/ubuntu/borderline-angehoerige/outputs/phase-8-rediagnose';

mkdirSync(OUT_DIR, { recursive: true });

// ─── Schritt 1: Whitespace-Bug Verstehen ──────────────────────────────────────
async function diagnoseWhitespaceBug(page) {
  await page.goto(BASE_URL + '/verstehen', { waitUntil: 'networkidle', timeout: 30000 });
  ...
  const elementsInRange = await page.evaluate(() => {
    ...
    function walk(el, depth) {
      ...
      if (bottom > 400 && top < 1400 && rect.width > 0) {
        const cn = typeof el.className === 'string' ? el.className.substring(0, 120) : '';
        results.push({
          tag: el.tagName,
          className: cn,
          top: Math.round(top),
  ...
  const contentChildren = await page.evaluate(() => {
    ...
    for (const child of main.children) {
      ...
      if (rect.width > 100) { contentDiv = child; break; }
    }
    if (!contentDiv) return [];

    // mx-auto div
    const mxAuto = contentDiv.querySelector('.mx-auto') || contentDiv.firstElementChild;
    if (!mxAuto) return [];
    ...
    for (const child of mxAuto.children) {
      ...
      const top = rect.top + window.scrollY;
      const bottom = rect.bottom + window.scrollY;
      const cn = typeof child.className === 'string' ? child.className.substring(0, 120) : '';
      ...
      results.push({
        ...
        id: child.id || '',
        // Heading text if any
        heading: child.querySelector('h1,h2,h3')?.textContent?.substring(0, 60) || '',
      });
      prevBottom = bottom;
    }
    return results;
...
async function diagnoseSidebar(page, slug, path) {
  ...
  const sidebarData = await page.evaluate(() => {
    ...
    const tocNav = document.querySelector('nav[aria-label*="Inhalt"], nav[aria-label*="content"], nav[aria-label*="toc"]');
    
    function elInfo(el) {
      if (!el) return null;
      const rect = el.getBoundingClientRect();
      const style = window.getComputedStyle(el);
      const cn = typeof el.className === 'string' ? el.className.substring(0, 200) : '';
      return {
        tag: el.tagName,
        className: cn,
        width: Math.round(rect.width),
    ...
    const allFixed = Array.from(document.querySelectorAll('[class*="fixed"]'))
      .filter(el => {
        ...
        return typeof cn === 'string' && cn.includes('left-') && cn.includes('top-');
      })
      .map(elInfo);
...
async function diagnoseSoforthilfe(page) {
  ...
  page.on('response', async resp => {
    if (resp.url().includes('soforthilfe') || resp.url().includes('index.html')) {
      responses.push({ 
        url: resp.url(), 
        status: resp.status(), 
  ...
  await page.goto(BASE_URL + '/soforthilfe', { waitUntil: 'networkidle', timeout: 30000 });
  await page.waitForTimeout(1000);
  ...
  const domInfo = await page.evaluate(() => {
    ...
    return {
      ...
      rootFirstChildClass: root?.firstElementChild?.className?.substring(0, 100) || '',
      hasMain: !!main,
      ...
      scriptTags: Array.from(document.querySelectorAll('script[src]')).map(s => s.src).filter(s => s.includes('assets/')).slice(0, 3),
...
async function diagnoseHomeVisual(page) {
  ...
  for (const vp of viewports) {
    ...
    const metrics = await page.evaluate((vpWidth) => {
      ...
      const images = Array.from(document.querySelectorAll('img, [class*="bg-[url"]')).filter(img => {
        const rect = img.getBoundingClientRect();
        return rect.top < window.innerHeight && rect.width > 0;
      }).length;

      // Drop-Cap
      const dropCap = document.querySelector('[class*="drop-cap"], [class*="first-letter"], .prose > p:first-child::first-letter');
...
(async () => {
  ...
  console.log('Content-Kinder auf Verstehen:', whitespaceData.contentChildren.length);
  
  // Grosse Gaps finden
  const bigGaps = whitespaceData.contentChildren.filter(c => c.gap_from_prev > 100);
  console.log('Grosse Gaps (>100px):', bigGaps.length);
  bigGaps.forEach(g => console.log(`  Gap ${g.gap_from_prev}px vor "${g.heading || g.className.substring(0,50)}" (top=${g.top})`));
  ...
  console.log('Requests:', soforthilfeData.requests.map(r => r.url));
  console.log('Responses:', soforthilfeData.responses.map(r => `${r.status} ${r.url}`));

  console.log('\n=== Schritt 4: Visuelle Wirkung Home ===');
  const homeVisual = await diagnoseHomeVisual(page);
  homeVisual.forEach(r => {
    console.log(`  ${r.viewport.name}: contentWidth=${r.metrics?.contentWidth}px, ratio=${r.metrics?.contentRatio}%, leftMargin=${r.metrics?.leftMargin}px, images=${r.metrics?.imagesAboveFold}`);
  });
  ...
  writeFileSync(join(OUT_DIR, 'rediagnose-results.json'), JSON.stringify(allResults, null, 2));
  console.log('\nErgebnisse gespeichert in:', OUT_DIR + '/rediagnose-results.json');
